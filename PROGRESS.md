...
- [x] Proxy URL 快速填写与凭据解析的真实页面/API 验证
- [x] Profile 紧凑刷新区、可选列及固定操作列的真实页面验证
- [x] Profile 详情页、维护/脚本/导航标签、共享操作与未保存草稿保护
- [x] Remote Tab 0.1.24 指针反馈及输入队列优化的业务集成与真实链路验证
- [x] Worker 媒体探测启动异常与取消清理修复、正式镜像及真实会话恢复验证

## 原生输入与紧凑页面

- [ ] Viewer 页眉整合线路状态与响应式真实页面验证
- [ ] Worker/Profile 存储保护列按内容布局验证
- [ ] Noto CJK 比例字体镜像、既有 Profile 字体更新与真实中文渲染
- [ ] Remote Tab 0.1.25 拖选、编辑键、剪贴板快捷键及原生文件拖入业务验收
