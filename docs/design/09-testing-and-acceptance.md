...
### Profile 管理工作流与远程指针（2026-09-09）

Portal/Backend/Gateway 使用 BrowShare `570b5cf`，Worker 使用包含后续修复的 `497445f`，
共同配对 Remote Tab `f7d897e`（0.1.24、wire 1.5）。干净公开源码通过正式候选工具构建、
校验 OCI、SBOM 和来源记录后部署，沿用原 Worker 身份、数据库、Profile 卷及测试 Extension
签名密钥。两个仓库的固定提交托管 CI 通过；npm、GHCR 和公开签名产物仍是独立发布步骤。

真实 Chrome 页面及 Backend API 验证以下工作流：

- Proxy URL 快速填写支持 HTTP/HTTPS/SOCKS5、显式端口、IPv6 和百分号编码凭据；非法 URL
  不覆盖表单，成功填写后清空原始 URL，凭据保持遮罩。实际创建和删除了独立测试 Proxy。
- Profile 列表合并刷新入口，默认五列；可选列在刷新后保留，重置有效，操作列固定右侧。
  列选择器支持键盘打开、Escape 关闭并返回焦点，390px 视口不产生页面横向溢出。
- 点击名称进入详情，基本信息和共享操作保留在顶部，维护、Page Script、导航策略使用可
  直接访问的子路由。实际编辑保存成功；两个脚本编辑器的未保存草稿在取消离开时保留。
  从真实 Viewer 返回准确的 Profile 维护详情；维护请求断网时保留详情并提供重试，恢复
  网络后重试成功。中英文、移动布局及限定 WCAG 2/2.1 A/AA 的 axe 检查无 violation，
  颜色对比仍保留人工确认项，不等同全站无障碍认证。

真实 Linux Chrome 152.0.7977.75、已激活的 Extension/Core 0.1.24 和本机 Chrome Viewer
验证了十个指针场景：文本框、链接、可编辑区域、开放 Shadow DOM、跨域 iframe 输入和链接、
返回主文档、父页面遮罩覆盖 iframe 及解除遮罩。实际本机浮点坐标经过 Viewer 输入映射，
文本/链接/等待/默认指针符合命中元素，中文输入由远端页面的真实 input 事件确认。跨域页面
来自另一个实际 HTTP origin，没有伪造光标事件。公共协议枚举、旧能力协商与输入队列边界
由 Remote Tab 的对应验证覆盖；本批不宣称重新执行四浏览器和全部 TURN 矩阵。

### 媒体对比及 Worker 启动失败修复（2026-09-09）

在同一服务器、实际 1280×713 viewport、1280×720/30fps/3000kbps 上限、关闭音频及相同
UDP relay 候选路径下，对旧版 0.1.23 和新版 0.1.24 分别采集静止、连续输入、滚动各 30 秒
真实接收端 RTCStats。每段预留 5 秒稳定时间，编码均为 VP8；码率仅计算视频 RTP 接收字节，
不含信令、TURN/IP 开销或业务 HTTP 请求。

| 场景 | 0.1.23 视频 kbps / 解码 fps | 0.1.24 视频 kbps / 解码 fps |
| --- | --- | --- |
| 静止 | 25.5 / 6.2 | 22.4 / 5.8 |
| 连续输入 | 47.9 / 10.0 | 50.4 / 10.6 |
| 滚动 | 673.1 / 14.4 | 806.2 / 16.1 |

两轮各 150 次实际输入完整到达，60 次滚动返回起点，媒体轨道保持相同、解码帧持续增加，
各采样区间视频丢包增量为 0。新版滚动码率和帧率均更高，不能由此推导普遍节省带宽；本次
未修改编码/画质策略，也未测量物理输入到显示延迟或发布端编码耗时。待处理的无按键 hover
合并只减少过时移动的排队，不合并拖拽、按键、点击或滚轮，详见 Remote Tab 输入契约。

第一轮候选媒体启动暴露 Worker 未捕获的 offer 等待超时，进程退出一次且非 OOM。原因是
同步启动抛错后跳过 await，遗留 promise 的定时器随后拒绝。`497445f` 将启动和 offer 等待
纳入同一受控调用，保留原始启动错误，并在成功、失败、关闭和取消时释放监听与定时器。
实际 Node 严格未处理拒绝模式下，旧实现退出码 1，新实现退出码 0；八条隔离生产代码路径
验证零遗留监听/定时器和零未处理拒绝。这是 Node 生命周期证明，不冒充真实 Chrome 故障注入。

Worker 修复镜像部署后，测试 Profile 还残留此前异常退出的旧容器 Chrome 单实例锁。确认旧
容器不存在、精确 Profile 无 Chrome 进程且持有独占文件锁后，仅清理该测试 Profile 的三个
Singleton 符号链接；没有处理用户 Profile，也不将此记为自动跨主机锁恢复。随后真实维护
会话完成上述媒体和光标验证，Worker 重启计数保持 0，关闭和删除均经业务 API 收敛。

证据保留在忽略的 `tmp/workspace-experience-qa/`、`tmp/media-offer-lifecycle-qa/` 以及
Remote Tab 的 `tmp/cursor-feedback-qa/`。失败原始记录与成功复测分开保留。测试 Session
均终态，测试 Profile 和 Proxy 已删除且 Profile 目录消失；本机浏览器、目标 HTTP 服务、
预览和构建器关闭。测试站保留六个业务容器，Worker 就绪且活动 Tab 为 0，用户原有 `test`
Profile 为 RUNNING，原数据盘未改动。
