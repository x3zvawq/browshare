...
<template>
  ...
  >
    <div class="route-heading">
      <span>{{ t('runtimeRoute.title') }}</span>
      <NTag size="small" :bordered="false" :type="healthType">
        {{ t(inactive ? 'runtimeRoute.inactive' : `runtimeRoute.${health?.status ?? 'UNKNOWN'}`) }}
      </NTag>
    </div>
    <small v-if="health && !inactive" :title="time(health.checkedAt)">{{
      t('runtimeRoute.checked', { date: time(health.checkedAt) })
    }}</small>
    <small v-else-if="!inactive">{{ t('runtimeRoute.never') }}</small>
    <small v-if="health?.lastSucceededAt && health.status === 'UNHEALTHY' && !inactive">{{
      t('runtimeRoute.succeeded', { date: time(health.lastSucceededAt) })
    }}</small>
